...
contract IdleJuniorAdapterTest is AbstractAdapterTest {
  ...
  function setUp() public {
    ...
    testConfigStorage = ITestConfigStorage(address(new IdleTestConfigStorage()));
  ...
  function verify_adapterInit() public override {
    ...
    assertEq(
      ...
      string.concat("Popcorn IDLE", IERC20Metadata(address(asset)).name(), " Adapter"),
      "name"
    );
    assertEq(
      IERC20Metadata(address(adapter)).symbol(),
      string.concat("popI-", IERC20Metadata(address(asset)).symbol()),
      "symbol"
    );
  }
