...
import {IIdleCDO, IRegistry} from "../IIdleCDO.sol";
import {IPermissionRegistry} from "../../../../interfaces/vault/IPermissionRegistry.sol";

/**
 * @title   Idle Junior Adapter
 * @author  0xSolDev
 * @notice  ERC4626 wrapper for Idle Vaults.
 *
 * An ERC4626 compliant Wrapper for https://app.idle.finance/#/earn/yield-tranches.
 * Allows wrapping IDLE Vaults with junior tranches.
 */
contract IdleJuniorAdapter is AdapterBase {
    ...
    function initialize(
        ...
        if (cdo.paused()) revert PausedCDO(_cdo);
        if (cdo.token() != asset()) revert NotValidAsset(asset());
        ...
        _symbol = string.concat("popI-", IERC20Metadata(asset()).symbol());
    ...
    function convertToUnderlyingShares(uint256, uint256 shares) public view override returns (uint256) {
        ...
        return supply == 0 ? shares : shares.mulDiv(balance, supply, Math.Rounding.Up);
    ...
    function _protocolWithdraw(
        ...
        cdo.withdrawBB(underlayingShares);
    }
